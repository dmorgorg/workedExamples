...
export default [
	...
	[
		`A rectangular channel with a base of ${ki(" 6.25\\,\\mathsf{m} ")} has a depth of ${ki(
			"y = 1.650\\, \\mathsf{m} "
		)} 
        when the discharge (volume flow rate) is ${ki(" Q=23.5 \\,\\mathsf{m^3\\!/s} ")}. 
	...
	[
		`A rectangular flume is built out of wood (${ki("n=0.012")}) and has a base width of ${ki(
			"1.250\\,\\mathsf{m}"
		)}. 
        The flume has a longitudinal slope of ${ki("0.14\\%")} and a flow depth of ${ki(
			"950\\,\\mathsf{mm}"
		)}. 
	...
	[
		...
		`For discharge ${ki(
			...
			"1.900\\,\\mathsf{m/s}"
		)}. If the channel is ${ki("4.50\\,\\mathsf{m}")} in width, determine ${ki("Q")}.`,
	...
	[
		...
		`<div class="inline-block width50 ">A triangular flume, built from unplaned wood ${ki(
			...
			"45^{\\circ}"
		)} to the horizontal (i.e., ${ki("z=1")}). If the slope of the flume is ${ki(
			"S=0.001"
	...
	[
		...
		// [
		// 	`A concrete-lined trapezoidal channel has a bed width of ${ki('6.00\\,\\mathsf{m}')} and side slopes of ${ki('z=0.5')}, a longitudinal slope of ${ki('1')} in ${ki('850')} and a Manning's coefficient of ${ki('n=0.014')}. Calculate the discharge and average flow velocity for normal flow with a depth of ${ki('2.30\\,\\mathsf{m}')}. Classify this flow.<br/> <div class="width55"><img src="/images/trapezoidalGuidedPractice2.png" /></div><div class="clear" />`,
		...
		// [
		// 	`A rough-formed concrete-lined trapezoidal channel ${ki('(n=0.015)')} has a bed width of ${ki('3.50\\,\\mathsf{m}')} and side slopes of ${ki('3H\\!:2V')} (i.e., three horizontal units for each two vertical units, or ${ki(' &lbrace; z=1.5}')}). The bed slope is ${ki('S=0.05\\%')}. Determine the depth ${ki('y')} when the discharge is ${ki(' &lbrace; Q=32.0\\,\\mathsf{m^3/s}}')}.<br/><br/> <div class="width70"><img src="/images/trapezoidalGuidedPractice1.png" /></div><div class="clear" />`,

		// 	[
		// 		[
		// 			`Use Manning's Equation to develop an expression for ${ki('Q')} as a function of ${ki('y')}.`,
		...
		// 		[
		// 			`Find ${ki('y')} that yields a discharge of ${ki('Q=32.0\\,\\mathsf{m^3/s}')}`,

		// 			`We are looking for ${ki('y')} that satisfies: $$32.0 = 1.4907 \\cdot \\frac{\\left( 3.50y+1.5y^2\\right)^{5/3}}{\\left(3.50+3.6056y\\right)^{2/3}}$$
		//                 This expression cannot be solved analytically for ${ki('y')}. Classically, this problem would be solved using 'trial-and-error', evaluating the right hand side of the expression for different values of ${ki('y')} until the corresponding discharge of ${ki('Q=32.0\\,\\mathsf{m^3\\!/s}')} is found. Trial-and-error remains a valid approach (but is time-consuming when precision is required).<br/><br/>
		//                 Starting with an initial guess of ${ki('y=1.000\\,\\mathsf{m}')}:
		//                 $$ \\begin{aligned}
		...
		//                 \\end{aligned}$$
		//                 This is clearly quite labour intensive. The calculations can be made easier using a spreadsheet but, if you have access to a spreadsheet, the spreadsheet's Goal Seek function will get you to the required value for ${ki('y')} much more readily.<br/><br/>
		//                 Fortunately, many inexpensive scientific calculators include a numeric solver which is the most convenient way to solve the expression for ${ki('y')}.`,
	...
	[
		...
		[
			...
			[
				`${ki("y=1.219\\,\\mathsf{m}")} (Pipe has maximum velocity at just over 80% depth.)`,
			...
			[
				`${ki("y=1.230\\,\\mathsf{m}")} (Pipe has same discharge as when full at 82% depth.)`,
			...
			[
				`${ki("y=1.407\\,\\mathsf{m}")} (Pipe has maximum discharge at 93.8% depth.)`,
