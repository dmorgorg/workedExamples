...
export default [
    ...
    y^3 - 2.1080y^2+0.88310 = 0.}`)} We can perform synthetic division to express the cubic equation as a product of a linear and a quadratic:<p></p>
       ...
       `)}
       will provide the provide two remaining roots, ${ki(`y_2`)} and ${ki(`y_3`)}, of the cubic:
       ${kd(`
       \\begin{aligned}
       ...
       \\end{aligned}
       `)}
        Discard the negative depth and the alternate depth is ${ki(`0.83181\\,\\mathsf{m}`)}.
        <blockquote>
    ...
    ${ki(`p`)}, ${ki(`q`)} and ${ki(
        ...
        (We didn't really need to use the coefficients of ${ki(
					`y^0`
				)} above but it does verify the earlier results from the higher powers of ${ki(`y`)}.)
        <p></p>
        Again,
        ${kd(`
        \\begin{gather*}
        ...
        \\end{gather*}
        `)}
        Now proceed as in Method 2 above to find  ${ki(`y_2=0.83188\\,\\mathsf{m}`)}.
    `,
		...
		`)} `,
          ...
          \\end{aligned}
        `)}
        For the same volume flow rate, flow depths of ${ki(`1.8500\\,\\mathsf{m}`)} and ${ki(
					`0.83188\\,\\mathsf{m}`
	...
	[
      ...
      A triangular flume, built from unplaned wood ${ki('(n=0.013)')}, has a design flow of ${ki(
			'3.25\\,\\mathsf{m^3\\!\\!/s}'
		)}. The sidewalls of the flume are inclined at ${ki('45^{\\circ}')} to the horizontal (${ki(
			'{z=1}'
		)}). If the slope of the flume is ${ki('{S=0.001,}')} determine the depth ${ki(
			'y'
	...
	[
      ...
      A triangular channel with sides inclined at ${ki(
			...
			'1.900\\,\\mathsf{m}'
		)} and the channel slope is ${ki('0.500\\%')}. At what slope will this flow become critical?
	...
	[
		...
		`A concrete-lined trapezoidal channel has a bed width of ${ki(
			'6.00\\,\\mathsf{m}'
		)} and side slopes of ${ki('z=0.5')}, a longitudinal slope of ${ki('1')} in ${ki(
			'850'
		)} and a Manning's coefficient of ${ki(
			'n=0.014'
		...
		`${kd(`Q=52.2\\,\\mathsf{m^3/s}, v=3.18\\,\\mathsf{m/s}, \\textsf{flow is sub-critical.}`)}`,
	],
	...
	has a bed width of ${ki('3.50\\,\\mathsf{m}')} and side slopes of ${ki(
	 ...
	 `,
		...
		[
			[
				`Use Manning's Equation to develop an expression for ${ki('Q')} as a function of ${ki(
					'y'
				)}.`,
    ...
    Given that the length of the channel is ${ki(
	 ...
	 `,
        ...
        ${kd(`
          ...
          \\end{aligned}
        `)}
        Find the depth ${ki('{y}')} of flow that has an area of ${ki('{35.833\\,\\mathsf{m^2}}')},
        ${kd(`
          \\begin{aligned}
            A &=by + zy^2 \\\\
        ...
        `)}`,
			...
			[
				`Find the slope ${ki(`S`)} that will generate a flow of ${ki('{35.833\\,\\mathsf{m^2}}')}`,
        ...
        `)}`,
			...
			[
				...
				`As noted, the the required flow of ${ki(
					'35.833\\,\\mathsf{m^3/s}'
				)} can be achieved with a depth of ${ki('y=2.5865\\,\\mathsf{m}')} and a slope of ${ki(
					`S=0.00010592`
        ...
        ${kd('h = 21.7\\,\\mathsf{m}-7.7854\\,\\mathsf{m}=13.915\\,\\mathsf{m}')}`,

				`${ki(`h=13.915\\,\\mathsf{m}`)}`,
			],
		],

		`${kd(`y=2.59\\,\\mathsf{m},\\quad S=0.01059\\%,\\quad h=13.92\\,\\mathsf{m}`)}`,
	],
